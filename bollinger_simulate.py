...
def valuesharp(ldt_timestamps, arg3 ):
    c_dataobj = da.DataAccess('Yahoo')
    ls_keys = ['open', 'high', 'low', 'close', 'volume', 'actual_close']
    ls_symbols = ['$SPX']
    ldf_data = c_dataobj.get_data(ldt_timestamps, ls_symbols,ls_keys)
    ...
    reader= csv.reader(open(arg3,'rb'),delimiter=',')
    i=0
    for key in reader:
...
def eventfind(ldt_timestamps ,arg2 ):
    ...
    for s_sym in ls_symbols:
        ...
        for i in range(0, len(ldt_timestamps)):           
            if sym_band[i] <= -2.0 and sym_band[i-1] >= -2.0 and spy_band[i] >= 1.0:
